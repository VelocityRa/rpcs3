...
namespace rsx
{
	namespace overlays
	{
		...
		void perf_metrics_overlay::update(bool force_update)
		{
			...
			if (elapsed >= m_update_interval || force_update)
			{
				f32 fps{0};
				f32 frametime{0};
				...
				{
					frametime = force_update ? 0 : std::max(0.0, elapsed / m_frames);
				...
				case detail_level::minimal:
				{
					fps = force_update ? 0 : std::max(0.0, static_cast<f32>(m_frames) / (elapsed / 1000));
				}
				}
